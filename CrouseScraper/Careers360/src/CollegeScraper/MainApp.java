...
public class MainApp {
    ...
    public static void main(String[] args) throws IOException {
            ...
            while (hasData) {
                ...
                for (Element card : collegeCards) {
                    ...
                    if (url.equals(urlString + "/colleges/list-of-bca-mca-colleges-in-india" + "?page=" + pageNum)
                            || url.equals(urlString + "/colleges/list-of-hospitality-tourism-colleges-in-india"
                                    + "?page=" + pageNum)
                            || url.equals(urlString + "/colleges/list-of-media-journalism-colleges-in-india" + "?page="
                                    + pageNum)
                            || url.equals(
                                    urlString + "/colleges/list-of-commerce-colleges-in-india" + "?page=" + pageNum)) 
                    {
                        
                        collegeName = card.select("h3.college_name").text();
                        collegeType = card.select(
                                "#college-list > div > div > div.facet-tuple-inner-container > div.left_side_tupple.w-100.listing-facet-tupple > div.tupple_top_block > div.tupple_right_block.d-none.d-lg-block > div > div > span:nth-child(1) > p.college-listing-rating")
                                .text() + " College";
                    ...
                    if (!collegeName.isEmpty())
                        excelExporter.addRow(sheet, rowNum++, collegeType, collegeName, "", collegeImage, "");
                }
            }
        ...
        excelExporter.saveAndClose("new_college_data.xlsx");
    }
}
